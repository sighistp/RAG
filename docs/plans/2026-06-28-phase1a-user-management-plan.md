...
3. 修改 `rag/auth.py` 的 `decode_token()` 或新增验证函数：

```python
def validate_token(token: str, password_changed_at: float = None) -> dict | None:
    """验证 token，检查是否因密码修改而失效。"""
    payload = decode_token(token)
    if not payload:
        return None
    # NULL 跳过检查（迁移后不踢用户）
...
4. 在需要验证 token 的地方调用 `validate_token()`
...
def test_admin_reset_password(db):
    """admin 重置密码成功。"""
    uid = db.create_user("alice", "OldPass1")
    admin = db.create_user("admin", "AdminPass1")
    db.set_user_admin(admin, True)
    db.reset_password(admin, uid, "NewPass1")
    user = db.authenticate("alice", "NewPass1")
    assert user is not None

def test_non_admin_reset_password_fails(db):
    """非 admin 重置密码应失败。"""
    uid = db.create_user("alice", "OldPass1")
    other = db.create_user("bob", "BobPass1")
    with pytest.raises(PermissionError):
        db.reset_password(other, uid, "NewPass1")
```

2. 跑测试确认 FAIL

3. 实现 `user_db.reset_password()` 方法

4. 实现 API 端点 `PUT /users/{uid}/reset-password`
...
**文件：** `frontend/src/stores/chat.ts`、`frontend/src/views/FileModeView.vue`、`frontend/src/views/KBModeView.vue`
